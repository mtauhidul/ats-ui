...
export default function KanbanWorkflowManagement() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      // Get stage names for history entry
      const oldStage = workflow?.stages?.find(s => s.id === oldStageId)?.name || 'Unassigned';
      const newStage = workflow?.stages?.find(s => s.id === newStageId)?.name || 'Unassigned';
      
      // Create history entry
      const historyEntry = {
        date: new Date().toISOString(),
        note: `Stage changed from "${oldStage}" to "${newStage}" via kanban board`,
      };

      // Update database with history
      await updateDoc(doc(db, 'candidates', candidateId), {
        stageId: newStageId,
        history: [...(currentCandidate.history || []), historyEntry],
        updatedAt: new Date().toISOString()
      });
