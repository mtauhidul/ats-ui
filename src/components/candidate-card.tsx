import { Mail, Phone, MapPin, Briefcase, GraduationCap, Calendar, Award } from "lucide-react";
...
export function CandidateCard({ candidate, onClick }: CandidateCardProps) {
  ...
  const daysSinceApplied = Math.floor((new Date().getTime() - new Date(candidate.createdAt).getTime()) / (1000 * 60 * 60 * 24));
      ...
      <CardContent className="p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {(candidateData.currentTitle || candidateData.currentCompany) && (
                <p className="text-xs text-muted-foreground truncate">
                  ...
                  {candidateData.currentCompany && ` at ${candidateData.currentCompany}`}
                </p>
              )}
            </div>
          </div>
          <Badge className={cn("text-xs px-2 py-1 border", statusColors[status as keyof typeof statusColors] || statusColors.active)}>
            {status.replace(/_/g, ' ')}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-3">
          ...
          {candidateData.experience && candidateData.experience.length > 0 && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <span>{candidateData.experience.length} position{candidateData.experience.length > 1 ? 's' : ''}</span>
            </div>
          )}
        </div>
        ...
        {candidateData.skills && candidateData.skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {candidateData.skills.slice(0, 5).map((skill: string, index: number) => (
          ...
          <div className="flex flex-col gap-1.5 flex-1">
            ...
            {aiScore !== undefined && (
              <div className="flex items-center gap-2 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-medium text-foreground">AI Score: {aiScore}%</span>
                </div>
                {recommendation && (
                  <Badge 
                    variant={recommendation === 'excellent_fit' ? 'default' : recommendation === 'good_fit' ? 'secondary' : 'outline'}
                    className="text-[10px] px-1.5 py-0.5"
                  >
                    {recommendation.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                  </Badge>
                )}
              </div>
            )}
            ...
            {strengths.length > 0 && (
              <div className="text-xs text-muted-foreground">
                <span className="font-medium">Strength:</span> {strengths[0]}
              </div>
            )}
            ...
            {candidateData.education && candidateData.education.length > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-muted-foreground">{candidateData.education[0].degree}</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            {candidateData.certifications && candidateData.certifications.length > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-muted-foreground">{candidateData.certifications.length} certs</span>
              </div>
            )}
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
