...
export function EditJobModal({
  ...
  // Helper function to transform backend job data to frontend format
  const transformJobToFormData = (jobData: Job): UpdateJobRequest => {
    const backendJob = jobData as unknown as Record<string, unknown>;
    
    // Parse location string to Address object
    let locationObj: { city: string; country: string } = { city: "", country: "" };
    if (typeof backendJob.location === 'string') {
      const parts = backendJob.location.split(',').map((p: string) => p.trim());
      locationObj = {
        city: parts[0] || "",
        country: parts[1] || "",
      };
    } else if (backendJob.location && typeof backendJob.location === 'object') {
      const loc = backendJob.location as { city?: string; country?: string };
      locationObj = {
        city: loc.city || "",
        country: loc.country || "",
      };
    ...
    // Note: At this point, normalizeJob() has already transformed the data
    // So jobData.requirements is already an object with experience and skills
    // We can use it directly
    const requirementsObj = jobData.requirements || {
      experience: "",
      skills: {
        required: [],
        preferred: [],
    ...
    return {
      ...
      workMode: (backendJob.locationType as WorkMode) || jobData.workMode || "hybrid",
  ...
  const [formData, setFormData] = useState<UpdateJobRequest>(transformJobToFormData(job));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      if (newErrors.title || newErrors.description || newErrors.locationCity || newErrors.locationCountry) {
        setCurrentTab("basic");
    ...
    const locationString = typeof formData.location === 'string' 
      ? formData.location 
      : `${formData.location?.city || ""}, ${formData.location?.country || ""}`.trim().replace(/^,\s*|,\s*$/, '');
  ...
  const handleAddSkill = () => {
    setFormData({
      ...formData,
      requirements: {
        ...(formData.requirements || { experience: "", skills: { required: [], preferred: [] } }),
        skills: {
          ...(formData.requirements?.skills || { required: [], preferred: [] }),
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        className={`h-11 ${errors.locationCity ? "border-red-500" : ""}`}
                      />
                      {errors.locationCity && (
                        <p className="text-sm text-red-500 flex items-center gap-1">
                          <span className="text-xs">⚠</span> {errors.locationCity}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="e.g., USA"
                        className={`h-11 ${errors.locationCountry ? "border-red-500" : ""}`}
                      />
                      {errors.locationCountry && (
                        <p className="text-sm text-red-500 flex items-center gap-1">
                          <span className="text-xs">⚠</span> {errors.locationCountry}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <Card className="border-dashed">
                    <CardContent className="p-4">
                      {(!formData.requirements?.skills?.required || formData.requirements.skills.required.length === 0) ? (
